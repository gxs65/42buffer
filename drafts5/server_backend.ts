...
class Paddle
{
	...
	process_keys(input: Client_events)
	{
		this.moving = 0;
...
class Pong_game
{
	...
	terminated: boolean; // game definitively stopped
	player_ws_ids: Array<ws_identifier> = [];
	players_input: Array<Client_events> = []; // 2 objects storing input received from players
	// Game state
	active: boolean = false; // game temporarily stopped
	...
	// Receives the user events as a JSON string sent by a player through websockets
