...
import { Client_commands, Client_message, Server_message,
		 Game_state_description, Game_init_description, Dimensions, Game_close_description } from "../shared/pong_interfaces.js";
import { lerp, ws_safe_send_client } from "../shared/pong_utils.js";
...
class Pong_game
{
	// Game state
	ws: WebSocket;
	status: "waiting" | "starting" | "active" | "paused" | "terminated"; // ("starting" status never used)
	...
	dims: Dimensions = {cwidth: 0, cheight: 0, pwidth: 0, pheight: 0, ball_radius: 0}
	// HTML page elements
	...
	bot_checkbox: HTMLInputElement | null = null;

	// SETUP

	// Constructor : Game starts NOT running (<status> is "waiting")
	constructor()
	{
		this.status = "waiting";
		...
		{
			this.abort_game_client("invalid HTML");
			return ;
		}
	...
	extract_HTML_elements() : boolean
	{
		...
		this.bot_checkbox = document.querySelector("#testbot");
		if (!this.announcement || !this.points_info || !this.bot_checkbox
	...
	// Called when the server sends game start message containing game setup data :
	...
	// When the server sends the game start message, stores the setup info,
	// and launches the animation loop (player can start to move its paddle)
	handle_start_by_server(gid: Game_init_description)
	{
		if (this.status != "waiting")
		{
			console.log("Server start message received after game started");
			this.abort_game_server("incoherent server message order");
			return ;
		}

		this.player_index = gid.player_index;
		this.server_broadcast_interval = gid.broadcast_interval;
		this.dims = gid.dims;
		this.adjust_canvas_dimensions();
		console.log(`-> server init message, assigned index ${this.player_index}, starting anim`);
		this.status = "active";
	...
	// When server side sends end message, terminates the game on client side
	// and displays the information on the page (reason of game end, winner)
	handle_end_by_server(gcd: Game_close_description)
	{
		let intro_string: string;

		if (!(this.status == "active" || this.status == "paused"))
		{
			console.log("Server end message received while game terminated");
			return ;
		}

		console.log(`-> Server end message, gave reason ${gcd.reason}, terminating game`);
		this.status = "terminated";
		intro_string = `[Player ${this.player_index}] Game ended by ${gcd.reason},`;
		if (gcd.winner == 0 || gcd.winner == 1)
			this.announcement!.innerHTML = `${intro_string} victory : player ${gcd.winner}.`;
		else
			this.announcement!.innerHTML = `${intro_string} players tied.`;
		this.points_info!.innerHTML = `Points : ${gcd.points[0]} - ${gcd.points[1]}`;
	}

	// When an error occurs and is caught on client side, ends game "unexpectedly"
	abort_game_client(reason: string)
	{
		console.log(`Game aborted because of client issue : ${reason}`)
		if (this.status == "terminated")
			return ;
		this.status = "terminated";
		this.close_websocket();
		if (this.announcement != null)
			this.announcement.innerHTML = "Game aborted because of client issue";
	}

	// When server produces unexpected behavior, ends game "unexpectedly"
	// Potential causes :
	// 		\ websocket connection to server closed
	// 		\ long time without any message from server
	// 		\ incoherent order of server message (eg. game state description before start message)
	abort_game_server(reason: string)
	{
		console.log(`Game aborted because of server issue : ${reason}`);
		if (this.status == "terminated")
			return ;
		this.status = "terminated";
		this.close_websocket();
		if (this.announcement != null)
			this.announcement.innerHTML = "Game aborted because of server issue";
	}
	...
	setup_websocket()
	{
		this.ws.onopen = () => {this.websocket_opened()};
		this.ws.onmessage = (event) => {this.websocket_receive(event.data);};
		this.ws.onclose = () => {this.websocket_closed()};
	}

	// Closes websocket if it is open
	close_websocket()
	{
	...
	// When connection to server succeeds, send first message for game choice
	websocket_opened()
	{
		...
		if (ws_safe_send_client(this.ws, message_string))
			this.abort_game_server("unable to send through ws");
	}

	// Registers data sent by the server and updates <time_last_data_received>,
	// using a `switch` statement to handle the different types of data
	// in the <Server_message> discriminated union
	// 		- (gcd) first data sent by server should be game setup data, indicating game start
	// 		- (gsd) following data should be game state description
	// 		- (gcd) last data should indicate end of the game and define the winner
	websocket_receive(data: string)
	{
		...
		try
		{
			...
			{
				case "gid":
					this.handle_start_by_server(message.data);
					break;
				case "gsd":
					this.handle_gsd_by_server(message.data);
					break;
				case "gcd":
					this.handle_end_by_server(message.data);
					break;
			}
		}
		catch
		{
			console.log(`Error while parsing data received from server`);
			this.abort_game_client("server message parsing");
		}
		this.time_last_data_received = performance.now();
	}

	// When websocket is closed by server, checks if it is unexpected
	// 		- if game is terminated, server must close websocket -> not an error
	// 		- otherwise, this is an error and the game should be aborted
	websocket_closed()
	{
		if (this.status == "terminated")
			console.log("Client websocket : server closed websocket as expected");
		else
			this.abort_game_server("websocket closed by server");
	}

	// Stringifies the data about recent user commands (pause/play/concede, paddle moves)
	// and sends it to the server through the websocket
	send_user_commands()
	{
		let message: Client_message = {
			id: "cc",
			data: this.commands_for_server
		};
		let message_string: string;
	...
	// GAME MAINLOOP : RECEIVE GAME STATE FROM SERVER, DISPLAY IT AS ANIMATION

	// When server broadcasts a new game state (the <gsd> object),
	// updates the state of the game (position of game components, points, status)
	// 		- if game state not defined on client side (<=> first time the server broadcasts it),
	// 			current game state becomes identical to description sent by server
	// 		- otherwise, description sent by server is stored in <game_state_next>
	// 			so that the <update_animation> loop can progress towards it with interpolation
	// 		- points and status are always updated
	handle_gsd_by_server(gsd: Game_state_description)
	{
		if (!(this.status == "active" || this.status == "paused"))
		{
			console.log("Server gsd message received while game terminated");
			this.abort_game_server("incoherent server message order");
			return ;
		}

		this.status = gsd.status;
		if (this.game_state_current == null) // initialization of <game_state_current>
			this.game_state_current = gsd;
		this.game_state_current.points = gsd.points;
		this.game_state_next = gsd;
	}
	...
	update_animation = () =>
	{
		...
		if (this.status == "terminated") // ends animation loop when game is terminated
			return ;
		...
		if (this.status == "active" && this.game_state_current != null)
		{
			...
			this.points_info!.innerHTML = `Points : ${this.game_state_current.points[0]} - ${this.game_state_current.points[1]}`;
		}
		if (this.status != "active")
		...
		if (this.bot_checkbox != null && this.bot_checkbox.checked)
			this.bot_decision_making();
		if (this.pending_commands_for_server)
			this.send_user_commands();
		...
		// (3) checks if no new data was received from server for a long time,
		//     which means that the server is down (but the websocket somehow remained open)
		if (current_time - this.time_last_data_received > 3000)
			this.abort_game_server("no recent server message");
		
		// (4) calls itself again with <requestAnimationFrame>
		requestAnimationFrame(this.update_animation);
	}
	...
	draw_canvas(gsd: Game_state_description)
	{
		...
		for (paddle_index = 0; paddle_index < 2; paddle_index++)
		{
			...
			ctx.fillRect(gsd.paddles_pos[paddle_index].x - this.dims.pwidth / 2,
				gsd.paddles_pos[paddle_index].y - this.dims.pheight / 2,
				this.dims.pwidth, this.dims.pheight);
		}
	...
	bot_decision_making()
	{
		...
		if (this.status != "active" || gsn == null)
			return ;
...
+ utiliser comme cote server une seule variable pour le statut de la partie 
+ verifier si le serveur n'a rien envoye depuis longtemps -> peut-etre down
