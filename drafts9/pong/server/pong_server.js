...
import { schema_client_message } from "../shared/pong_interfaces.js";
...
export class Pong_game {
    ...
    handle_player_commands(commands, player) {
...
class Pong_manager {
    ...
    new_player(ws_id) {
        ...
        ws_id.socket.on("message", (msg) => { this.ws_handle_data(msg, ws_id); });
    ...
    game_instance_terminated(ws_id1, ws_id2) {
    ...
    waiting_game_instance_terminated(ws_id1) {
...
class Transcendance_server {
    ...
    constructor(hostname, port) {
        ...
        this.ws_server.on("connection", (ws) => this.new_player(ws));
    ...
    new_player(ws) {
        ...
        ws.once("message", (msg) => { this.ws_devolve_player(msg, ws_id); });
...
+ definir la structure propre de creation et de fermeture des parties
    ...
    + quand la partie se termine (premier a 10 points) :
        <Pong_game.end_by_points_or_concession> -> <Pong_server.game_instance_terminated> -> <Pong_server.erase_game>
    \ quand Pong_server rencontre une erreur fatale (pas encore defini car structure de serveur par encore tres claire)
...
- separer le serveur en deux classes : 1 pour le service HTTP general, 1 pour les WS et creation de games de Pong
    (+ eventuellement une classe de serveurs pour les games de chess ?)
...
On a completely different note : I will play a game of Twilight Imperium in two days, as the faction Titans of Ul and third in turn order for the first round. I got a rather poor slice except it has planet Hope's end on an equidistant system between me and my Naaz-Rokha neighbor. My other neighbor is the rather scary Sardakk Norr. How should I play this game ? Try not answering with trivialities.
