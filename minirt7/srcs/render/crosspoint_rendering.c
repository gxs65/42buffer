...
/*   Updated: 2025/03/14 17:50:42 by abedin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	ft_color_ray_at_object_lambertian(t_data *data, t_ray ray, t_intersection *intersect)
{
	...
	while (current_light)
	{
		...
		{
			current_color =
				ft_vec_scale_by_vec(intersect->object->albedo,
	...
	if (1)
	{
		...
		color_components = ft_vec_add(color_components,
			ft_vec_scale_by_vec(ft_draw_one_ray(data, diffusion_ray), intersect->object->albedo));
		data->recurr_lvl--;
		if (g_log)
		{
	...
	{
		color_components = ft_vec_add(ft_vec_scale(ft_vec_scale_by_vec(intersect->object->albedo, data->alightning.albedo), data->alightning.dist),
			ft_vec_scale(color_components, 1 - data->alightning.dist));
	}
	else
	{
		color_components = ft_vec_add(color_components,
			ft_vec_scale(ft_vec_scale_by_vec(intersect->object->albedo, data->alightning.albedo), data->alightning.dist));
	}
