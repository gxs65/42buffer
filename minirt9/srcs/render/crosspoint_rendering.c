...
/*   Updated: 2025/03/17 15:27:07 by abedin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	ft_color_ray_at_object_lambertian(t_data *data, t_ray ray, t_intersection *intersect)
{
	...
	if (g_log & B_COLORING)
	{
		printf("\t\t[COLORING] normal vector at intersection point : ");
	...
	while (current_light)
	{
		...
		if (g_log & B_COLORING)
		{
			printf("[COLORING] For computing mirrored component : light direction, mirrored light direction, negative current ray direction :\n");
			ft_vec_display(light_direction, 1);
			ft_vec_display(mirrored_light_direction, 1);
			ft_vec_display(ft_vec_neg(ray.dir), 1);
			printf("[COLORING] + sproduct of negative ray and mirrored light, power, final : %f  ; %f ; %f\n", sproduct2, a, b);
		}
		...
		if (!ft_check_ray_shadows(data, shadow_ray, &shadow_interval) && sproduct > 0)
		{
			...
			else if (g_log & B_COLORING)
				printf("[COLORING] Mirrored component dropped for being opposed to ray direction\n");
	...
	if (g_log & B_COLORING)
	{
		printf("[COLORING] Color components after applying direct illumination : ");
		ft_vec_display(color_components, 1);
	}

	// Contribution to coloring from indirect illumination (light received from surfaces that reflected light to this surface) : one random ray sampled
	// -> ray simulates diffusion skewed towards normal (diffusion vector randomly taken on the unit sphere, then offset from crosspoint by normal)
	if (1)
	{
		...
		if (g_log & B_COLORING)
		{
			printf("[COLORING] Color components after applying INdirect illumination : ");
			ft_vec_display(color_components, 1);
		}
	}
	...
	//if (intensity < data->alightning.dist)
	{
		...
		if (g_log & B_COLORING)
		{
			printf("[COLORING] Color components after applying ambient lighting : ");
			ft_vec_display(color_components, 1);
		}
	}
