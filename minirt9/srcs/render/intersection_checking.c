...
/*   Updated: 2025/03/17 14:34:42 by abedin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	ft_draw_one_ray(t_data *data, t_ray ray)
{
	...
	if (g_log)
	{
		printf("[RAY] Calculating for new ray, recurr_level = %d : ", data->recurr_lvl);
	...
	{
		if (object_closest)
			printf("[RAY] Ray intersected object %p at t = %f, calculating color\n", object_closest, t_min);
		else
	...
	if (g_log)
	{
		printf("[RAY] -> ray color returned at recurr_lvl %d : ", data->recurr_lvl);
...
int	ft_check_ray_shadows(t_data *data, t_ray ray, t_interval *hit_interval)
{
	...
	if (g_log & B_SHADOWS)
	{
		printf("[SHADOWS] Checking shadows on interval %f - %f for 1 ray :\n", hit_interval->min, hit_interval->max);
		ft_ray_display(ray, 1);
	}
	object_current = data->objects;
	while (object_current)
	{
		...
		{
			if (g_log & B_SHADOWS)
				printf("\t[SHADOWS] shadow ray did intersect object %p within interval\n", object_current);
			return (1);
		}
		object_current = object_current->next;
	}
