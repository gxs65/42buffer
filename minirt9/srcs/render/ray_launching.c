...
/*   Updated: 2025/03/17 15:17:58 by abedin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_calc_sample_one_pixel(t_data *data, int pixel_x, int pixel_y)
{
	...
	color = ft_vec_scale(color_components, (double)1 / (double)data->nb_samples_per_pixel);
	color_int = ft_assemble_color(color);
	if (g_log)
	{
		printf("== Point %d'%d\n", pixel_x, pixel_y);
