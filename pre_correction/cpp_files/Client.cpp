...
void	Client::preparePollFd(struct pollfd& pfd)
{
	...
	{
		this->_response->defineCGIPollfd(pfd);
	}
	else
	{
...
int		Client::sendHTTP(void)
{
	...
	if (nBytesWritten == 0)
		return (logError("[CH::receiveHTTP] 0 bytes written to socket despite POLLOUT", 1));
...
int	Client::finishRequestHeaders(char *buffer, size_t bufferSize, ssize_t indHeadersEnd)
{
	...
	if (this->_request->parse(this->_requestHeaders))
	{
		...
		this->handleProblematicRequest("400 Bad Request (missing/incoherent/malformed headers)", 1);
		return (0);
	}
	...
	if (this->_request->_hasBody && (this->_request->_chunkedBody || this->_request->_bodySize != 0))
	{
