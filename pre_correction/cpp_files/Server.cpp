...
// /!\ Special case : actually fd of <pfds> surveiled by <poll> are not necessarily fd of sockets,
// 	   when a <Client> instance waits for a CGI child to terminate it is a pipe fd (cf <Client>)
...
int		Server::serverLoop(void)
{
	...
	while (1)
	{
		...
		if (nfds == std::string::npos) // allocation error
			return (this->stopServer(), 1);
		...
		if (nfdsReady > 0 && this->goThroughEvents(pfds, nfds)) // should never happen
			return (delete[] pfds, this->stopServer(), 1);
...
// 		(+ try/catch block to check for the case where an allocation would fail)
...
int		Server::handleOldConnection(unsigned int socketInd, struct pollfd& pfd)
{
	...
	std::cout << "\tClient socket fd " << pfd.fd << " of client ind " << clientInd << " is ready, events : ";
	this->displayEvents(pfd.revents);
	try
	{
	...
	{
		std::cout << "[handleOldConnection] Alloc exception during handling of event, deleting instance of <Client>";
		ret = 1;
	}
	if (ret == 1 || ret == 2)
	{
...
// 		(but we do not need to keep it in memory ourselves, it is managed by the OS)
int		Server::handleNewConnection(unsigned int mainSocketInd)
{
	...
	std::cout << "\tMain socket ind " << mainSocketInd << " ready to accept new connection\n";
	if (this->_clientHandlers.size() >= MAX_SIMULTANEOUS_CLIENTS)
	{
		std::cout << "[handleNewConnection] Cannot accept new connection : simultaneous clients limit reached\n";
		return (0);
	}
	...
	{
		return (logError("[handleNewConnection] Allocation error when creating new client", 1));
	}
	return (0);
}
...
// Returns the total size of the array <pfds>, or std::string::npos in case of an error
unsigned long		Server::createPollFds(struct pollfd **pfds)
{
	unsigned long	nfds = this->_mainSockets.size() + this->_clientHandlers.size();
	unsigned long	ind;
	unsigned long	clientInd;
	...
	{
		std::cout << "[createPollFds] Alloc exception while creating pollfd array";
