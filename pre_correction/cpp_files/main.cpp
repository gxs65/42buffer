...
/*   Updated: 2025/05/21 14:34:16 by abedin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
		...
		cfgFileName = "cfg_base.conf";

	Server		serv;
	if (serv.initServer(cfgFileName))
		return (1);
	if (serv.startServer())
		return (1);
	if (serv.serverLoop())
...
- to explain during correction
	...
	\ non-blocking I/O : <poll> used to make sure that fds that could block are ready before I/O -> fd of sockets and pipe, not of regular files
- to show during correction
	...
	\ handling UPLOAD : raw, multipart/form-data, chunked bodies on upload page
						modify max body size to be able to upload images
...
+ correction methode DELETE
+ proteger les derniers new
+ dernier fix sur le write dans le pipe du CGI
+ petit script tests Python
