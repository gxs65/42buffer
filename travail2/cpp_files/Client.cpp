...
int		Client::sendHTTP(void)
{
	...
	nBytesWritten = send(this->_socketFd, this->_responseBuffer + this->_nbytesSent,
											this->_responseSize - this->_nbytesSent, 0);
	if (nBytesWritten <= 0)
...
// 			checking for errors <=> malformed request
...
// 		\ since request bytes are read part by part, separator '\r\n\r\n' may be split on 2 buffers,
// 			so we have to maintain a memory of the last buffer in <_lastBuffer>,
