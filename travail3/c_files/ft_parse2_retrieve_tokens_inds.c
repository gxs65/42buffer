...
void	ft_parse2_tokens_ind_par_retrieve(t_data *data, t_node *current,
	int opar, int cpar)
{
	int		ind_redir;
	int		ind;
...
void	ft_parse2_tokens_ind_leaf_count(t_data *data, t_node *current)
{
	int	ind;
...
void	ft_parse2_tokens_ind_leaf_retrieve(t_data *data, t_node *current)
{
	int		ind;
	int		ind_cmd;
	int		ind_redir;
...
{
	ft_printf(LOGSV, "[PARSE2] \tReading redir tokens of leaf node\n");
	ft_parse2_tokens_ind_leaf_count(data, current);
	...
	if (current->nb_redir_tokens > 0)
		current->redir_tokens_inds
			= malloc(current->nb_redir_tokens * sizeof(int));
	ft_parse2_tokens_ind_leaf_retrieve(data, current);
	return (0);
}
