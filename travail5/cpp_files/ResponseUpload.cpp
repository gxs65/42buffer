...
// The real "upload" following a POST/PUT request :
// opens a new file at <newPath>, and dumps all the content of <buffer> into it
...
// Checks if file at <fullPath> exists (before writing the whole <fileContent> into it)
// to write the right HTTP status code in the response (201 for 'created', 200 for 'replaced')
...
// Extracts the boundary string from the header "Content-Type",
// and puts it in an allocated array of char
// (prepending two '-' because real boundary in body will have those two '--')
...
// Handles one part of a multipart body in the buffer <part> :
// 		\ searches for the part headers and extracts the file's filename from it
// 		\ uploads the part body as a new file in directory <dirPath>
...
// Goes through the body of a multipart POST request, searching for the <boundary>
// to extract parts and handle them separately
...
// Handles a POST request with a multipart body : body is made of multiple parts,
// separated by a <boundary> given in "Content-Type" header, each with its own headers and (sub)body
// 		-> the filenames of the (potentially numerous) files are in part headers,
// 		   the request path must only give the path to the upload directory
...
// Handle a POST request with he raw content of the file to upload as body
// 		-> the filename of the unique uploaded file must be the end of the request path
