...
int	invalidReturnCode(std::string& codeLit)
{
	...
	for (ind = 0; ind < codeLit.size(); ind++)
	{
		if (!(codeLit[ind] >= '0' && codeLit[ind] <= '9'))
			return (1);
	}
...
///////////////////////////////////////
// VERSERS STRUCTURES INITIALIZATION //
///////////////////////////////////////
...
void	logVserver(t_vserver& vs)
{
	...
	std::cout << "listenAddress : " << vs.listenAddress
		<< "\nlistenPort : " << vs.listenPort
		<< "\nmaxRequestBodySize : " << vs.maxRequestBodySize << "\n";
...
int	invalidVserverParameters(t_vserver& vs)
{
	...
	{
		for (ind2 = 0; ind2 < vs.locations.size(); ind2++)
		{
			if (ind1 != ind2 && vs.locations[ind1].locationPath.compare(vs.locations[ind2].locationPath) == 0)
...
// 		\ call <parseBlock> to parse a 'location' block
int parseLineServ(std::string& line, std::ifstream& inStream, void* storage)
{
	...
	{
		if (tokens.size() != 2 || vserv->listenSet)
		...
		if (tokensListen.size() == 2 || tokensListen[0].find('.') == std::string::npos)
			vserv->listenPort = static_cast<uint16_t>(strtol(tokensListen[tokensListen.size() - 1].c_str(), NULL, 10));
		if (tokensListen.size() == 2 || tokensListen[0].find('.') != std::string::npos)
		{
			if (inet_aton(tokensListen[0].c_str(), &translatedIPAddress) == 0)
				return (logError("[serv] invalid value for 'listen' directive", 1));
			vserv->listenAddress = ntohl(translatedIPAddress.s_addr);
		}
		vserv->listenSet = 1;
	}
