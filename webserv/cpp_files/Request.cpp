...
/*   Updated: 2025/05/21 14:29:59 by abedin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_location*	Request::identifyLocation()
{
	...
	std::cout << "[Res::idLoc] Finding location responsible for request with path "
		<< this->_filePath << " in vserver " << this->_vserv << "\n";
	bestMatch = NULL;
	...
	std::cout << "\tbestMatch is : " << (bestMatch ? bestMatch->locationPath : "NULL") << "\n";
	return (bestMatch);
}
...
bool	Request::parseFirstLine(const std::string& rawHeaders)
{
	...
	if (!isalpha(this->_method[0])) // #f : to be removed, present only to correct a strange added character from tester
		this->_method.erase(this->_method.begin());
...
bool	Request::checkHeaders()
{
	...
	if (this->_headers.count("Content-Length") && this->_headers.count("Transfer-Encoding"))
		return (logError("[Req::parse] Error: Content-Length defined for chunked body", 0));
...
// (each header may be present only once, even header "Cookie", hence the use of a map)
	...
	while (line_start < rawHeaders.size())
	{
		...
		if (colon_pos != std::string::npos)
		{
			...
			if (this->_headers.count(key))
				return (logError("[Req::parse] Error: same header defined twice", 0));
...
int	Request::methodForbidden()
{
	...
	if (this->_location)
	{
		std::cout << "\tchecking if method " << this->_method << " " << this->_method.size()
			<< " is accepted : at location " << this->_location
			<< ", GET accepted : " << this->_location->acceptedMethods.count("GET")
			<< ", POST accepted : " << this->_location->acceptedMethods.count("POST") << "\n";
	}
