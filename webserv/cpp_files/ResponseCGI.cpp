...
// 		\ redirect 'stdin' (request body received from <tempFilePath>)
...
void	Response::childExecuteCGI(std::string& fullPath, std::string& interpreterPath, int fdOutput)
{
	char**			argv;
	char**			envArray;
	...
	std::cout << "CGI CHILD : executes with input from temp file " << this->_tempFilePath
	...
	if (this->_request->_method == "POST" && this->_request->_bodySize > 0)
		fdInput = open(this->_tempFilePath, O_RDONLY);
	else
		fdInput = open("/dev/null", O_RDONLY);
	if (fdInput < 0)
	{
		logError("CGI CHILD : Error: 'open' on temp file failure", 1);
		exit(127);
	}
...
// When there is a request body, sends it to the child on <fd> (the sending end of <pipeRequest>)
// by parts of size <WRITEPIPE_BUFFER_SIZE> until the whole body has been sent
// Closes the given <fd> after all the request body is sent
int		Response::feedBodyToChild(int fd)
{
	long			nBytesWritten;
	...
	while (posInBody < bodySize)
	{
		...
		nBytesWritten = write(fd, this->_request->_body + posInBody, nBytesToWrite);
		if (nBytesWritten <= 0)
			return (close(fd), 1);
...
// Called by the <Client> instance when it must prepare its poll fd while in WAITING_CGI state
// Fills the <pfd> with POLLIN and the fd of the pipe to receive CGI output
void	Response::defineCGIPollfd(struct pollfd& pfd)
{
	if (this->_cgiPipeState == RECEIVING_OUTPUT)
	{
		pfd.fd = this->_cgiReadFd;
		pfd.events = POLLIN;
	}
	else
	{
...
// Called by <Client> instance when POLLIN event occurred
// on <_cgiReadFd> where CGI output is expected
// 2 options :
// 		\ reading on <_cgiReadFd> returns positive number
// 			-> CGI child still alive, so store the read bytes and continue to wait for output (return code 3)
// 		\ read on fd returns 0 (EOF)
// 			-> CGI child closed its end of the output pipe <=> it terminated,
...
int		Response::checkCGIFinished()
{
	...
	std::cout << "[Res::CGI] reading on CGI output pipe\n";
	...
	{
		std::cout << "[Res::CGI] read a positive number of bytes, continuing to wait\n";
		this->_cgiOutput.insert(this->_cgiOutput.end(), buffer, buffer + nBytesRead);
		return (3);
	}
}

// Stores the child process' PID and fd of the pipes (used to send/receive from child) in member variables
...
int		Response::startWaitCGI(pid_t pid, int cgiReadFd)
{
	std::cout << "\tCGI child successfully forked\n";
	this->_cgiPid = pid;
	this->_cgiReadFd = cgiReadFd;
	this->_cgiPipeState = RECEIVING_OUTPUT;
	std::cout << "\tCGI child PID " << this->_cgiPid << ", receiving output on pipe fd " << this->_cgiReadFd << ", starting wait\n";
	return (3);
}
...
// 		\ a temp file to send the request body (stdin of CGI child)
...
int		Response::forkCGI(std::string& fullPath, std::string& interpreterPath)
{
	...
	// Transmit request body into temp file
	if (this->_request->_method == "POST" && this->_request->_bodySize > 0)
	{
		bzero(this->_tempFilePath, PATH_MAX);
		this->_tempFileFd = this->createTempFile(this->_tempFilePath);
		if (this->_tempFileFd == -1)
			return (logError("Error: unable to create temp file for CGI input\n", 1),
				this->makeErrorResponse("500 Internal Server Error (system call)"));
		if (this->feedBodyToChild(this->_tempFileFd))
		{
	...
	{
		close(pipeCGIout[0]);
		this->childExecuteCGI(fullPath, interpreterPath, pipeCGIout[1]);
		return (0);
	}
	else
	{
		close(pipeCGIout[1]);
		return (this->startWaitCGI(pid, pipeCGIout[0]));
	}
}
