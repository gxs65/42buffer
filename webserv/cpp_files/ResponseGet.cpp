...
std::string	Response::getResponseBodyType(std::string& fullPath)
{
	size_t			ind;
	std::string		extension;

	ind = fullPath.find_last_of('.');
	if (ind == std::string::npos)
		extension = "";
	else
		extension = fullPath.substr(ind);
	if (extension == ".html")
		return ("text/html");
...
int	Response::makeFileResponse(std::string& fullPath, size_t fileSize, std::string status)
{
	...
	std::cout << "\tsending regular file at : " << fullPath << "\n"
		<< "\t\twith Content-Type : " << this->getResponseBodyType(fullPath) << "\n";
...
// Tries to serve an index file when a request targets directory at <fullPath>
...
// 		\ when no index file present and cfg file allowed auto indexing,
// 			respond with index file generated by <makeAutoindexResponse>
// 		\ in all other cases, respond with a 404 error
int	Response::tryServeIndex(std::string& fullPath)
{
	...
	if (stat(indexPath.c_str(), &fileStat) == 0 && S_ISREG(fileStat.st_mode))
	{
		...
		return (this->makeFileResponse(indexPath, fileSize, "200 OK"));
	}
	else if (this->_location->autoIndex)
	{
		std::cout << "\tpotential index file does not exist, auto index on\n";
...
// Performs checks before trying to serve an index file :
...
int	Response::handleGetOnDir()
{
	...
	if (ret == 0) // when the path ends with '/' but leads to a file and not a directory
	...
	return (this->tryServeIndex(fullPath));
}
