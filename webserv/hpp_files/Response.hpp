...
# define FEEDING_BODY 0
# define RECEIVING_OUTPUT 1
...
class Response
{
	public:
		...
		int				getCGIChildFd();
		void			defineCGIPollfd(struct pollfd& pfd);
	...
	private:
		...
		int					_cgiReadFd;
		int					_cgiPipeState;
		unsigned long		_nBytesSentToChild;
		...
		void				childExecuteCGI(std::string& fullPath, std::string& interpreterPath, int fdOutput);
		int					handleCGIFinished();
		int					feedBodyToChild(int fd);
		int					startWaitCGI(pid_t pid, int cgiReadFd);
		int					createTempFile(char* tempFilePath);
